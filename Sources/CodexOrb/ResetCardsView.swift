...
/// Read-only inventory. Each ticket represents one available reset.
@MainActor
final class ResetCardsView: NSView {
    ...
    override func resetCursorRects() {
        ...
        for index in 0..<max(0, self.credits?.availableCount ?? 0) {
            self.addCursorRect(self.ticketRect(index), cursor: .pointingHand)
        }
    }
    ...
    private func updateCardAppearance() {
        for (index, label) in self.subviews.enumerated() {
    ...
    override func mouseUp(with event: NSEvent) {
        // Visual press feedback only; no redemption action is connected.
        self.pressedCard = nil
        self.hoveredCard = self.cardIndex(at: self.convert(event.locationInWindow, from: nil))
        self.updateCardAppearance()
    }
    ...
    init(credits: CodexResetCredits?) {
        ...
        for index in 0..<count {
            ...
            let rect = self.ticketRect(index)
            let label = NSTextField(labelWithString: Self.countdown(expiration))
            label.font = .monospacedDigitSystemFont(ofSize: expiration.map { $0 <= Date() } == true ? 12 : 22, weight: .medium)
