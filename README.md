...
```sh
shasum -a 256 CodexOrb-0.1.5-macOS-arm64.zip
```
...
4. 悬停可展开，拖动可移动，点击配额环查看当前返回的额度窗口详情，点击令牌数查看用量明细。右键菜单可刷新、打开设置或退出；需要随系统启动时在设置中开启「开机启动」。
...
- 点击配额环可打开当前返回的额度窗口（5 小时、每周或 30 天）及重置时间详情。
- 重置预测刷新检测到 Tibo 承诺从无到有时，会在胶囊旁显示约 6 秒的「新的Tibo重置」小气泡，点击可打开额度详情卡片。首次加载只建立基线，请求失败或持续有承诺不会重复提示。
- 点击重置卡片堆可打开可用的重置卡片。
...
- 折叠状态：优先显示每周剩余配额，其次显示 5 小时或 30 天剩余配额；如果某个窗口没有返回则不显示它的占位内容。配额环上仍保留周额度节奏指示器；没有周窗口或周配额耗尽时隐藏该指示器。
...
会先选取 `codex` 配额桶，再按照返回的窗口时长匹配 300/10080/43200 分钟（5 小时、每周、30 天）；其他时长暂不展示。缺失的数据不会按零用量处理。
