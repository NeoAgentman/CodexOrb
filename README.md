...
```sh
shasum -a 256 CodexOrb-0.1.7-macOS-arm64.zip
```
...
- Tibo 承诺有有效数值时，整个胶囊显示红金色火焰与上升火星；承诺清空后熄灭，临时请求失败保留上次状态。火焰独立播放，不受系统「减少动态效果」设置影响。
- 点击重置卡片堆可打开可用的重置卡片。
...
设置中的「自动使用即将到期的重置卡」默认关闭，保存开启后生效。可选择到期前 **1–12 小时**，默认 **6 小时**。
默认只检查当前选中的账号；勾选「应用到所有账号」后，会逐个检查 CodexOrb 中添加的账号，无须切换当前账号。

应用运行时每分钟检查一次，启动、保存相关设置和电脑唤醒后也会检查。当实时查询确认周剩余额度低于 100%，
且最早到期的可用卡片距离到期不超过设置时间（包括不足一小时），会弹出 **30 秒倒计时**，统一列出本次符合条件的账号和各自卡片到期时间。
可以点击「立即使用」或「取消不使用」；不操作则倒计时结束后按列表逐个使用，每个账号一张。按钮作用于列表中的整批卡片。
发送前会再次查询并核对同一张卡和周额度；如果卡片变化或不再符合条件，会跳过该账号，不自动换用其他卡片。
取消或关闭倒计时框后，列表中的这些卡将不再自动弹出或使用（重启后仍有效），但可以手动兑换。
更改自动使用设置、切换账号、退出或电脑睡眠会中止当前倒计时；中止不会将卡片记为用户取消。
卡片详情不完整、到期时间未知、周额度数据缺失或无效时会跳过；单个账号失败不影响其他账号。

服务端仍可能返回 `nothingToReset`，这表示当前没有符合兑换条件的额度窗口。
每个账号每轮至多发起一次兑换；结果不确定或消费后额度刷新失败时，会保留待处理记录并暂停该账号的自动兑换，
可切换到该账号使用「恢复上次重置操作」。关闭功能或更改账号范围会取消尚未发送的操作，已发送的兑换无法撤回。
应用退出、电脑睡眠或断网期间不能保证及时使用。自动检查的结果写入 macOS 系统日志的 `CodexOrb / AutomaticReset` 分类。

## 重置卡片与确认

点击已知且可用的卡片后，会立即在胶囊旁打开紧凑的确认框，仅显示问题和账号。实时校验和进程等待会在确认后进行。
**手动兑换只有点击“确认”才会执行变更。取消、关闭和默认回车键都不会授权这次手动兑换。** 卡片排序时仍会保留卡片 ID。
